...
type
  Tfrmformatos = class(TForm)
    ...
    procedure dblcampoExit(Sender: TObject);
    procedure edtinicioExit(Sender: TObject);
    procedure edtinicioEnter(Sender: TObject);
    procedure edtlongitudExit(Sender: TObject);
  private
...
//  Grabar campo del formato del exctrato bancario
    ...
    begin
          ...
          begin
          ShowMessage('Seleccione el inicio del campo ');
          edtinicio.SetFocus;
          exit;
          end;
          ...
          DATAMODULE1.qryformatos.SQL.ADD('INSERT INTO FORMATOS_BANCOS_CONTRV(COD_BANCO,COD_CAMPO,DELIMITADOR,INI_CAMPO,LON_CAMPO,FORMATO,NUM_CAMPO,ESTADO)');
          DATAMODULE1.qryformatos.SQL.ADD('VALUES (:"COD_BANCO",:"COD_CAMPO",:"DELIMITADOR",:"INI_CAMPO",:"LON_CAMPO",:"FORMATO",:"NUMERO",:"ESTADO")');
          DATAMODULE1.qryformatos.Parameters[0].VALUE := dblbanco.KEYVALUE;
          DATAMODULE1.qryformatos.Parameters[1].VALUE := dblcampo.KEYVALUE;
          DATAMODULE1.qryformatos.Parameters[2].VALUE := inttostr(dbldelimitadores.ItemIndex);
          DATAMODULE1.qryformatos.Parameters[3].Value := STRTOINT(edtinicio.TEXT);
          DATAMODULE1.qryformatos.Parameters[4].VALUE := STRTOINT(edtlongitud.TEXT);
          DATAMODULE1.qryformatos.Parameters[5].VALUE := trim(msformato.text);
          DATAMODULE1.qryformatos.Parameters[6].VALUE := trim(edtnumcampo.text);
          IF  ckestado.Checked = TRUE THEN
          BEGIN
          DATAMODULE1.qryformatos.Parameters[7].VALUE := 1;
          END
          ELSE
          DATAMODULE1.qryformatos.Parameters[7].VALUE := 0;
          DATAMODULE1.qryformatos.ExecSQL;

          DATAMODULE1.qryformatos.CLOSE;
          DATAMODULE1.qryformatos.SQL.Clear;
          DATAMODULE1.qryformatos.SQL.ADD('SELECT * FROM FORMATOS_BANCOS_CONTRV WHERE COD_BANCO = :"CODIGO"');
         ...
         begin
              ...
              DATAMODULE1.qryformatos.CLOSE;
              DATAMODULE1.qryformatos.SQL.Clear;
              DATAMODULE1.qryformatos.SQL.ADD('UPDATE FORMATOS_BANCOS_CONTRV SET DELIMITADOR = :"DELIMITADOR",INI_CAMPO = :"INI_CAMPO" ,LON_CAMPO = :"LON_CAMPO",FORMATO = :"FORMATO",ESTADO = :"ESTADO"');
              DATAMODULE1.qryformatos.SQL.ADD('WHERE COD_BANCO = :"COD_BANCO" AND COD_CAMPO = :"COD_CAMPO"');
              DATAMODULE1.qryformatos.Parameters[5].VALUE := dblbanco.KEYVALUE;
              DATAMODULE1.qryformatos.Parameters[6].VALUE := dblcampo.KEYVALUE;
              DATAMODULE1.qryformatos.Parameters[0].VALUE := inttostr(dbldelimitadores.ItemIndex);
              DATAMODULE1.qryformatos.Parameters[1].Value := STRTOINT(edtinicio.TEXT);
              DATAMODULE1.qryformatos.Parameters[2].VALUE := STRTOINT(edtlongitud.TEXT);
              DATAMODULE1.qryformatos.Parameters[3].VALUE := trim(msformato.text);
              IF  ckestado.Checked = TRUE THEN
              BEGIN
              DATAMODULE1.qryformatos.Parameters[4].Value := 1;
              END
              ELSE
              DATAMODULE1.qryformatos.Parameters[4].VALUE := 0;
...
begin
              ...
              DATAMODULE1.qryformatos.CLOSE;
              DATAMODULE1.qryformatos.SQL.Clear;
              DATAMODULE1.qryformatos.SQL.ADD('SELECT * FROM FORMATOS_BANCOS_CONTRV WHERE COD_BANCO = :"COD_BANCO"');
              DATAMODULE1.qryformatos.Parameters[0].VALUE := TRIM(dblbanco.KEYVALUE);
              DATAMODULE1.qryformatos.Open;
              edtnumcampo.Text := inttostr(DATAMODULE1.qryformatos.RecordCount +1);
              DATAMODULE1.qryformatos.CLOSE;
              DATAMODULE1.qryformatos.SQL.Clear;
              DATAMODULE1.qryformatos.SQL.ADD('SELECT * FROM FORMATOS_BANCOS_CONTRV WHERE COD_BANCO = :"COD_BANCO" AND COD_CAMPO = :"COD_CAMPO"');
              DATAMODULE1.qryformatos.Parameters[0].VALUE := TRIM(dblbanco.KEYVALUE);
              DATAMODULE1.qryformatos.Parameters[1].VALUE := TRIM(dblcampo.KEYVALUE);
              DATAMODULE1.qryformatos.Open;
...
begin
        ...
        BEGIN
              DATAMODULE1.qryformatos.CLOSE;
              DATAMODULE1.qryformatos.SQL.Clear;
              DATAMODULE1.qryformatos.SQL.ADD('DELETE  FROM FORMATOS_BANCOS_CONTRV WHERE COD_BANCO = :"COD_BANCO"');
              DATAMODULE1.qryformatos.Parameters[0].VALUE := TRIM(dblbanco.KEYVALUE);
              DATAMODULE1.qryformatos.ExecSQL;
              dblbanco.Enabled :=false;
              dblcampo.KeyValue := '';
...
procedure Tfrmformatos.dblcampoExit(Sender: TObject);
begin
IF dbldelimitadores.ItemIndex = 3 THEN
BEGIN
...
procedure Tfrmformatos.edtinicioExit(Sender: TObject);
begin
IF edtlongitud.TEXT = '' THEN
edtlongitud.TEXT := '1';

end;

procedure Tfrmformatos.edtinicioEnter(Sender: TObject);
begin
edtinicio.Text  := inttostr(inicio);
end;

procedure Tfrmformatos.edtlongitudExit(Sender: TObject);
begin
IF edtinicio.TEXT = '' THEN
edtlongitud.TEXT := inttostr(inicio);

IF edtlongitud.TEXT = '' THEN
edtlongitud.TEXT := '1';

        inicio := strtoint(edtinicio.text)+ strtoint(edtlongitud.TEXT);
