...
var
  frmgdcorrecion: Tfrmgdcorrecion;
...
procedure Tfrmgdcorrecion.Button1Click(Sender: TObject);
...
begin
  ...
  begin
    ...
    if dmcompensa.qrymcdeclaracion3TIPO_CIERRE.Value = 'C' then
    begin
      ...
      begin
        ...
        begin
          dmcompensa.qrymcdecnumrad.Close;
          dmcompensa.qrymcdecnumrad.Prepare;
          dmcompensa.qrymcdecnumrad.Params[0].Value := dmcompensa.qrymcdeclaracion3NUM_DECLARATIVA.Value;
          dmcompensa.qrymcdecnumrad.Open;
          dmcompensa.qrymcdecnumrad.First;
          tcmcn := 0;
          tcmce := 0;
          tcmpn := 0;
          tcmpe := 0;
          ...
          writeln(outputfile,'');
          writeln(outputfile,'1.IDENTIFICACION DE LA EPS:REDSALUD ATENCION HUMANA EPS  2. DECLARACION                         3. PERIODOS');
          writeln(outputfile,'+-----------------------------------------------------+  +----------------------------------+   +---------------------------------------+');
          writeln(outputfile,'|1.1 Codigo               |                EPS035     |  |2.1 INICIAL              |   '+espacios(1,a)+'    |   |3.1 PERIODO COMPENSACION |   '+espacios(7,dmcompensa.qrymcdeclaracion3periodo_compensado.value)+'   |');
          writeln(outputfile,'|1.2 No. de la Declaracion|                  '+espacios(8,dec)+' |  |2.2 ADICION              |  '+espacios(1,b)+'     |   |         (MM/AAAA)       |             |');
          writeln(outputfile,'|1.3 Direc. Domicilio Prin| CLL 100 N 19-61 PISO 8    |  |2.3 CORRECCION           |   '+espacios(1,c)+'    |   |-------------------------+-------------|');
          writeln(outputfile,'|1.4 Telefono/Fax         | 6353459                   |  |2.3.1 REEMPLAZA A        |'+espacios(8,floattostr(dmcompensa.qrymcdecnumradNUM_DECLARATIVA.Value))+'|   |3.2 FECHA DECLARACION    |'+espacios(12,datetostr(dmcompensa.qrymcdeclaracion3fecha_declaracion.value))+' |');
          writeln(outputfile,'+-----------------------------------------------------+  |2.4 COMPLEMENTARIA       |   '+espacios(1,d)+'    |   |      (DD/MM/AAAA)       |             |');
          ...
          writeln(outputfile,'');
          writeln(outputfile,'');
          writeln(outputfile,'12. NOMBRE                   : DEYANIRA ORDOÑEZ DE GODOY');
          writeln(outputfile,'');
          writeln(outputfile,'');
          writeln(outputfile,'===================================================================================================================================================');
        end;
      ...
      showmessage('La Declaracion No Corresponde a Una Correccion');
  end;
  closefile (outputfile);
end;
