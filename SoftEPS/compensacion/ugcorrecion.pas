...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Gauges, Math;

type
  Tfrmcorrecion = class(TForm)
    Panel1: TPanel;
    Panel2: TPanel;
    Label3: TLabel;
    GroupBox2: TGroupBox;
...
procedure Tfrmcorrecion.Button1Click(Sender: TObject);
var
  periodo : string;
  fecha : string;
  i : real;
...
begin
  ...
  begin
    ...
    dmcompensa.tbldeclaracionescot_tipo_decla.Value := 4;
    dmcompensa.tbldeclaracionesperiodo_compensado.value := dmcompensa.qrymcdeclaracion3PERIODO_COMPENSADO.Value;
    dmcompensa.tbldeclaracionesfecha_declaracion.Value := dmcompensa.qrymcdeclaracion3FECHA_DECLARACION.Value;
    dmcompensa.tbldeclaracionestipo_cierre.Value := 'C';
    dmcompensa.tbldeclaracionesNUM_DECLARATIVA.Value := dmcompensa.qrymcdeclaracion3NUM_DECLARACION.Value;
    dmcompensa.tbldeclaraciones.Post;
    fecdec := dmcompensa.qrymcdeclaracion3FECHA_DECLARACION.Value;
    dmcompensa.tbldeclaraciones.Close;
    dmcompensa.qrymcdeclaracion1.Close;
    dmcompensa.qrymcdeclaracion3.Edit;
    dmcompensa.qrymcdeclaracion3NUM_DECLARATIVA.Value := strtofloat(edit2.text);
    dmcompensa.qrymcdeclaracion3.Post;
    dmcompensa.qrymcdeclaracion3.Close;
    dmcompensa.qrymcgcorr.Close;
    dmcompensa.qrymcgcorr.Prepare;
    dmcompensa.qrymcgcorr.Params[0].Value := edit1.Text;
    dmcompensa.qrymcgcorr.Open;
    dmcompensa.qrymcgcorr.First;
    while not dmcompensa.qrymcgcorr.Eof do
    begin
      dmcompensa.qrymcgcorr.Edit;
      dmcompensa.qrymcgcorrNUM_DECLARACION.Value := num_declara;
      dmcompensa.qrymcgcorr.Post;
      dmcompensa.qrymcgcorr.Next;
    end;
    dmcompensa.qrymcdecaficom1.Close;
    dmcompensa.qrymcdecaficom1.Prepare;
    dmcompensa.qrymcdecaficom1.Params[0].Value :=fecdec;
    dmcompensa.qrymcdecaficom1.Open;
    dmcompensa.qrymcdecaficom1.First;
    gauge1.MinValue := 0;
    gauge1.MaxValue :=dmcompensa.qrymcdecaficom1.RecordCount;
    gauge1.Progress := 1;
    frmcorrecion.Refresh;
    i := 0;
    while not dmcompensa.qrymcdecaficom1.Eof do
    begin
      ano1 :='01/01/'+copy(dmcompensa.qrymcdecaficom1periodo_compensado.Value,4,4);
      ...
      dmcompensa.qrymcestcos.Params[1].Value :=dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value;
      ...
      dmcompensa.qrymcbasesdecalculo.Params[0].Value := dmcompensa.qrymcdecaficom1num_declaracion.value;
      dmcompensa.qrymcbasesdecalculo.Params[1].Value := dmcompensa.qrymcdecaficom1cod_grupo_etareo.value;
      dmcompensa.qrymcbasesdecalculo.Open;
      if (dmcompensa.qrymcbasesdecalculo.RecordCount = 0) and (dmcompensa.qrymcdecaficom1tip_beneficiario.Value <> 'A') then
      begin
        ...
        dmcompensa.tblbasesdecalculonum_declaracion.Value := dmcompensa.qrymcdecaficom1num_declaracion.Value;
        dmcompensa.tblbasesdecalculocod_grupo_eta.Value := dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value;
        if (dmcompensa.qrymcdecaficom1tip_beneficiario.Value = 'C') and (dmcompensa.qrymcdecaficom1dias.Value = '30') and (dmcompensa.qrymcdecaficom1tip_upc.Value = 'N') then
        begin
          dmcompensa.tblbasesdecalculotot_cotizantes_mcn.Value := 1;
          dmcompensa.tblbasesdecalculotot_dias_cotznn.Value := strtofloat(dmcompensa.qrymcdecaficom1dias.Value);
          dmcompensa.tblbasesdecalculotot_afiliados_mcn.Value := 1;
        end;
        if (dmcompensa.qrymcdecaficom1tip_beneficiario.Value = 'C') and (dmcompensa.qrymcdecaficom1dias.Value = '30') and (dmcompensa.qrymcdecaficom1tip_upc.Value = 'E') then
        begin
          dmcompensa.tblbasesdecalculotot_cotizantes_mce.Value := 1;
          dmcompensa.tblbasesdecalculotot_dias_cotize.Value := strtofloat(dmcompensa.qrymcdecaficom1dias.Value);
        ...
        if (dmcompensa.qrymcdecaficom1tip_beneficiario.Value = 'B') and (dmcompensa.qrymcdecaficom1dias.Value = '30') and (dmcompensa.qrymcdecaficom1tip_upc.Value = 'N') then
        begin
          dmcompensa.tblbasesdecalculotot_beneficiarios_mcn.Value := 1;
          dmcompensa.tblbasesdecalculotot_dias_cotznn.Value := strtofloat(dmcompensa.qrymcdecaficom1dias.Value);
          dmcompensa.tblbasesdecalculotot_afiliados_mcn.Value := 1;
        end;
        if (dmcompensa.qrymcdecaficom1tip_beneficiario.Value = 'B') and (dmcompensa.qrymcdecaficom1dias.Value = '30') and (dmcompensa.qrymcdecaficom1tip_upc.Value = 'E') then
        begin
        ...
        if (dmcompensa.qrymcdecaficom1tip_upc.Value = 'N') and (dmcompensa.qrymcdecaficom1tip_beneficiario.Value <> 'A') then
        begin
          dmcompensa.tblbasesdecalculotot_upc_dzn.Value := Redondea(dmcompensa.qrymcvaloresval_upc_dia_normal.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.Value));
          dmcompensa.tblbasesdecalculotot_valor_upc.Value :=Redondea(dmcompensa.qrymcvaloresval_upc_dia_normal.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.Value));
          dmcompensa.qrymcvalupc.Close;
          dmcompensa.qrymcvalupc.Prepare;
          dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
          dmcompensa.qrymcvalupc.Params[1].Value :=dmcompensa.qrymcdecaficom1afi_tipo_doc.Value;
          dmcompensa.qrymcvalupc.Params[2].Value :=dmcompensa.qrymcdecaficom1afi_numero_doc.Value;
          dmcompensa.qrymcvalupc.Open;
          dmcompensa.qrymcvalupc.First;
          dmcompensa.qrymcvalupc.Edit;
          dmcompensa.qrymcvalupcval_upc.Value :=Redondea(dmcompensa.qrymcvaloresval_upc_dia_normal.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value));
          dmcompensa.qrymcvalupc.Post;
          dmcompensa.qrymcvalupc.Close;
        end;
        if (dmcompensa.qrymcdecaficom1tip_upc.Value = 'E') and (dmcompensa.qrymcdecaficom1tip_beneficiario.Value <> 'A') then
        begin
          dmcompensa.tblbasesdecalculotot_upc_dze.Value := Redondea(dmcompensa.qrymcvaloresval_upc_dia_especial.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.Value));
          dmcompensa.tblbasesdecalculotot_valor_upc.Value :=Redondea(dmcompensa.qrymcvaloresval_upc_dia_especial.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value));
          dmcompensa.qrymcvalupc.Close;
          dmcompensa.qrymcvalupc.Prepare;
          dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
          dmcompensa.qrymcvalupc.Params[1].Value :=dmcompensa.qrymcdecaficom1afi_tipo_doc.Value;
          ...
          dmcompensa.qrymcvalupc.First;
          dmcompensa.qrymcvalupc.Edit;
          dmcompensa.qrymcvalupcval_upc.Value :=Redondea(dmcompensa.qrymcvaloresval_upc_dia_especial.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value));
          dmcompensa.qrymcvalupc.Post;
          dmcompensa.qrymcvalupc.Close;
        end;
      ...
      begin
        ...
        begin
          ...
          dmcompensa.qrymcbasesdecalculotot_dias_cotznn.Value    := dmcompensa.qrymcbasesdecalculotot_dias_cotznn.Value+ strtofloat(dmcompensa.qrymcdecaficom1dias.value);
        ...
        begin
          ...
          dmcompensa.qrymcbasesdecalculotot_dias_cotize.Value    := dmcompensa.qrymcbasesdecalculotot_dias_cotize.Value+strtofloat(dmcompensa.qrymcdecaficom1dias.Value);
        ...
        if (dmcompensa.qrymcdecaficom1tip_upc.Value = 'N') and (dmcompensa.qrymcdecaficom1tip_beneficiario.Value <> 'A') then
        begin
          dmcompensa.qrymcbasesdecalculotot_upc_dzn.Value := Redondea(dmcompensa.qrymcbasesdecalculotot_upc_dzn.Value+(dmcompensa.qrymcvaloresval_upc_dia_normal.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value)));
          dmcompensa.qrymcbasesdecalculotot_valor_upc.Value :=Redondea(dmcompensa.qrymcbasesdecalculotot_valor_upc.Value+(dmcompensa.qrymcvaloresval_upc_dia_normal.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value)));
          dmcompensa.qrymcvalupc.Close;
          dmcompensa.qrymcvalupc.Prepare;
          dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
          dmcompensa.qrymcvalupc.Params[1].Value :=dmcompensa.qrymcdecaficom1afi_tipo_doc.Value;
        ...
        if (dmcompensa.qrymcdecaficom1tip_upc.Value = 'E') and (dmcompensa.qrymcdecaficom1tip_beneficiario.Value <> 'A') then
        begin
          dmcompensa.qrymcbasesdecalculotot_upc_dze.Value := Redondea(dmcompensa.qrymcbasesdecalculotot_upc_dze.Value+(dmcompensa.qrymcvaloresval_upc_dia_especial.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value)));
          dmcompensa.qrymcbasesdecalculotot_valor_upc.Value :=Redondea(dmcompensa.qrymcbasesdecalculotot_valor_upc.Value+(dmcompensa.qrymcvaloresval_upc_dia_especial.Value * dmcompensa.qrymcestcosval_estructura_cost.Value * strtofloat(dmcompensa.qrymcdecaficom1dias.value)));
          dmcompensa.qrymcvalupc.Close;
          dmcompensa.qrymcvalupc.Prepare;
          dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
          dmcompensa.qrymcvalupc.Params[1].Value :=dmcompensa.qrymcdecaficom1afi_tipo_doc.Value;
      ...
      fecagre := '01/'+dmcompensa.qrymcdecaficom1periodo_compensado.Value;
      if (dmcompensa.qrymcdecaficom1tip_beneficiario.Value = 'A') then
      begin
        dmcompensa.qrymcbasesdep.Close;
        dmcompensa.qrymcbasesdep.Prepare;
        dmcompensa.qrymcbasesdep.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
        dmcompensa.qrymcbasesdep.Params[1].Value :=dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value;
        ...
        begin
          ...
          dmcompensa.tblbasesdepnum_declaracion.Value := dmcompensa.qrymcdecaficom1num_declaracion.Value;
          dmcompensa.tblbasesdepcod_grupo_eta.Value := dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value;
          ...
          begin
            ...
            begin
              if dmcompensa.qrymcupczoncod_grupo_eta.Value = dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value then
              begin
                ...
                if dmcompensa.qrymcdecaficom1tip_upc.Value = 'N' then
                begin
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 6 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upc*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 5 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upc*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 4 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upc*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea(((upc*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end;
                  dmcompensa.qrymcvalupc.Close;
                  dmcompensa.qrymcvalupc.Prepare;
                  dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
                  ...
                  dmcompensa.qrymcvalupc.First;
                  dmcompensa.qrymcvalupc.Edit;
                  dmcompensa.qrymcvalupcval_upc.Value :=redondea(((upc*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  dmcompensa.qrymcvalupc.Post;
                  dmcompensa.qrymcvalupc.Close;
                  if (strtodate(fecagre) >= strtodate('01/03/2000')) then
                  begin
                    dmcompensa.tblbasesdepval_promocion.Value := redondea(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 6 then
                  begin
                    upcdep:= redondea((((upc*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 5 then
                  begin
                    upcdep:= redondea((((upc*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 4 then
                  begin
                    upcdep:= redondea((((upc*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  begin
                    upcdep:= redondea(((upc*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end;
                    promdep :=roundto(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value),0);
                ...
                if dmcompensa.qrymcdecaficom1tip_upc.Value = 'E' then
                begin
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 6 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upces*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 5 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upces*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 4 then
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea((((upces*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  begin
                    dmcompensa.tblbasesdepval_upc_dependientes.Value:= redondea(((upces*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end;
                  dmcompensa.qrymcvalupc.Close;
                  dmcompensa.qrymcvalupc.Prepare;
                  dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
                  ...
                  dmcompensa.qrymcvalupc.First;
                  dmcompensa.qrymcvalupc.Edit;
                  dmcompensa.qrymcvalupcval_upc.Value :=redondea(((upces*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  dmcompensa.qrymcvalupc.Post;
                  dmcompensa.qrymcvalupc.Close;
                  if (strtodate(fecagre) >= strtodate('01/03/2000')) then
                  begin
                    dmcompensa.tblbasesdepval_promocion.Value := redondea(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    upcdep :=redondea(((upces*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    promdep :=roundto(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value),0);
                    dmcompensa.tblbasesdepval_solidaridad.Value := redondea(((upcdep+promdep)*dmcompensa.qrymcupczonsol.Value));
                  end;
                  if (strtodate(fecagre) < strtodate('01/03/2000')) then
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficomcod_grupo_etareo.Value = 5 then
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficomcod_grupo_etareo.Value = 4 then
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea(((upc*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end;
                  dmcompensa.qrymcvalupc.Close;
                  dmcompensa.qrymcvalupc.Prepare;
                  dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
                  ...
                  if (strtodate(fecagre) >= strtodate('01/03/2000')) then
                  begin
                    dmcompensa.qrymcbasesdepval_promocion.Value := dmcompensa.qrymcbasesdepval_promocion.Value+redondea(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    if dmcompensa.qrymcdecaficomcod_grupo_etareo.Value = 6 then
                    begin
                      upcdep:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    end
                    else
                    if dmcompensa.qrymcdecaficomcod_grupo_etareo.Value = 5 then
                    begin
                      upcdep:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    end
                    else
                    if dmcompensa.qrymcdecaficomcod_grupo_etareo.Value = 4 then
                    begin
                      upcdep:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upc*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    end
                    else
                    begin
                      upcdep:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea(((upc*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                    end;
                    promdep:=roundto(((dmcompensa.qrymcupczonpyp.Value/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value),0);
                ...
                begin
                  ...
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 6 then
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upces*1.85)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 5 then
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upces*2.02)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  if dmcompensa.qrymcdecaficom1cod_grupo_etareo.Value = 4 then
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea((((upces*3)*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                  end
                  else
                  begin
                    dmcompensa.qrymcbasesdepval_upc_dependientes.Value:= dmcompensa.qrymcbasesdepval_upc_dependientes.Value+redondea(((upces*est/12)/30)*strtofloat(dmcompensa.qrymcdecaficom1dias.value));
                   end;
                  dmcompensa.qrymcvalupc.Close;
                  dmcompensa.qrymcvalupc.Prepare;
                  dmcompensa.qrymcvalupc.Params[0].Value :=dmcompensa.qrymcdecaficom1num_declaracion.Value;
      ...
      dmcompensa.qrymcbasesdep.Close;
      dmcompensa.qrymcbasesdecalculo.Close;
      dmcompensa.qrymcdecaficom1.Next;
    ...
    begin
      ...
      try
       ...
       begin
         ...
         begin
           if (dmcompensa.qrymcdeclaracion2TIPO_CIERRE.Value = 'C') and (dmcompensa.qrymcdeclaracion2NUM_DECLARATIVA.Value = strtofloat(edit1.Text)) then
           begin
           ...
           dmcompensa.Database1.StartTransaction;
      ...
      showmessage('Felicitaciones ! Ha Finalizado del Proceso de Correcion,Declaracion Generada'+floattostr(num_declara));
    end;
  end;
end;
