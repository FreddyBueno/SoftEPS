unit ugendeclaraciones;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls,DateUtils, IdTrivialFTPBase, Mask, ComCtrls, ExtCtrls;

type
  Tfrmgendeclara = class(TForm)
...
procedure Tfrmgendeclara.Button1Click(Sender: TObject);
...
//    filtro1 := copy(filtro,7,4)+'/'+copy(filtro,4,3)+copy(filtro,1,2);
    dmcompensa.qrymcdeclaracion2.Prepare;
    dmcompensa.qrymcdeclaracion2.Params[0].Value := strtodate(filtro1);
    ...
    begin
      ...
      begin
        ...
        tampn := 0;
        tampe := 0;
        dec:= floattostr(dmcompensa.qrymcdeclaracion2num_declaracion.value);
        a:= '';
        b:= '';
        c:= '';
        d:= '';
        e:= '';
        if (dmcompensa.qrymcdeclaracion2cot_tipo_decla.Value = 0) and (dmcompensa.qrymcdeclaracion2TIPO_CIERRE.Value = 'D') then
        begin
          a := 'X';
          b := '';
        end;
        if (dmcompensa.qrymcdeclaracion2cot_tipo_decla.Value <> 0) or (dmcompensa.qrymcdeclaracion2TIPO_CIERRE.Value <> 'D') then
        begin
          b := 'X';
          a := '';
        end;
//        if dmcompensa.qrymcdeclaracion2cot_tipo_decla.Value = 2 then
 //          c := 'X';
  //      if dmcompensa.qrymcdeclaracion2cot_tipo_decla.Value = 3 then
   //        d := 'X';
    //    if dmcompensa.qrymcdeclaracion2cot_tipo_decla.Value = 4 then
     //      e := 'X';
        anoestcos :=copy(dmcompensa.qrymcdeclaracion2periodo_compensado.value,4,4);
        ...
        begin
          ...
          begin
            ...
            dmcompensa.qrymcetabas.Params[0].Value :=dmcompensa.qrymcdeclaracion2num_declaracion.value;
        ...
        dmcompensa.qrymcliquidacion.Params[0].Value :=dmcompensa.qrymcdeclaracion2num_declaracion.Value;
        ...
        begin
          ...
          dmcompensa.qrymcbasesdep.Params[0].Value :=dmcompensa.qrymcdeclaracion2num_declaracion.value;
        ...
        writeln(outputfile,'+-----------------------------------------------------------------------------------------------+');
        writeln(outputfile,'|          TOTAL  |'+espacios(13,copy(format('%n',[tna]),1,length(format('%n',[tna]))-2))+'|'+espacios(16,format('%n',[tvu]))+'|'+espacios(24,format('%n',[tap]))+'|'+espacios(21,format('%n',[tas]))+'|');
        writeln(outputfile,'+-----------------------------------------------------------------------------------------------+');
        writeln(outputfile,'       10. INFORMACION ADICIONAL');
        writeln(outputfile,'+-----------------------------------------------------------------------+-------------+-------------------+');
        writeln(outputfile,'|10.1 No de Autoliquidaciones                         |'+espacios(17,copy(format('%n',[dmcompensa.qrymcliquidacionnum_autoliquidaciones.value]),1,length(format('%n',[dmcompensa.qrymcliquidacionnum_autoliquidaciones.value]))-3))+'|             |                   |');
        ...
        writeln(outputfile,'|10.4 No. Total de Afiliados inscritos                |'+espacios(17,copy(format('%n',[dmcompensa.qrymcliquidacionnum_tot_afil.value]),1,length(format('%n',[dmcompensa.qrymcliquidacionnum_tot_afil.value]))-3))+'|             |                   |');
        writeln(outputfile,'|10.5 No. Total de Cotizantes 8%                      |'+espacios(17,copy(format('%n',[dmcompensa.qrymcliquidacionnum_total_cotiz8.value]),1,length(format('%n',[dmcompensa.qrymcliquidacionnum_total_cotiz8.value]))-3))+'|             |                   |');
...
procedure Tfrmgendeclara.FormClose(Sender: TObject;
