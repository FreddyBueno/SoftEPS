...
type
  Tfrmafiliacion_usuarios = class(TForm)
    Panel1: TPanel;
    ...
    qry2: TQuery;
    qry2COD_PRESTADOR: TStringField;
    qry2MON_PRESTADOR: TStringField;
    qry2NUM_ACREDITACION: TStringField;
    qry2COD_DEPARTAMENTO: TStringField;
    qry2COD_CIUDAD: TStringField;
    qry2DIR_PRESTADOR: TStringField;
    qry2TEL_PRESTADOR: TStringField;
    qry2REP_LEGAL: TStringField;
    qry2NIV_ATEN_PRESTADOR: TStringField;
    qry2TIP_PRESTADOR: TStringField;
    qry2TIP_IDENTIFICACION: TStringField;
    qry2NUM_IDENTIFICACION: TStringField;
    qry2CLA_PRESTADOR: TStringField;
    qry2REC_COPAGO: TStringField;
    qry2REC_CUOTA: TStringField;
    qry2EST_PRESTADOR: TStringField;
    qry2FEC_ULTIMA_AUTO: TDateTimeField;
    qry2EST_CONTRATO: TStringField;
    qry2COD_REGINAL: TStringField;
    qry2COD_OFICINA: TStringField;
    qry2FEC_INGRESO: TDateTimeField;
    qry2DIR_ELECTRONICA: TStringField;
    Label37: TLabel;
    ...
    procedure DBLookupComboBox21Exit(Sender: TObject);
...
var
  frmafiliacion_usuarios: Tfrmafiliacion_usuarios;
...
procedure Tfrmafiliacion_usuarios.dsrelacion_afiliadosDataChange(
...
procedure Tfrmafiliacion_usuarios.qryrelacion_afiliadosAfterInsert(
  DataSet: TDataSet);
begin
     ...
     // qryrelacion_afiliadosfec_inclusion_nov.Value:=frmafiliaciones.fecha_hoy();
      ...
      qryrelacion_afiliadosFEC_INCLUSION_NOV.Value := strtodate('01'+copy(datetostr(frmafiliaciones.fecha_actual()),3,8));
...
procedure Tfrmafiliacion_usuarios.qryrelacion_afiliadosBeforePost(
  DataSet: TDataSet);
var
cod_novedad:string;
begin
        ...
        if qrybeneficiarioscod_tipo_afil.Text = 'C' then
            ...
            begin
                ...
                qrybeneficiariosfec_nacimiento.Value:=DateTimePicker5.date;
                if TabSheet1.TabVisible = true then
                  qrybeneficiariosfec_afiliacion_sis.value:=DateTimePicker7.Date
                else
                  qrybeneficiariosfec_afiliacion_sis.value:= strtodate('01'+copy(datetostr(frmafiliaciones.fecha_actual()),3,8));
                if TabSheet1.TabVisible = true then
                  qrybeneficiariosfec_afiliacion_uni.value:=DateTimePicker7.Date
                else
                  qrybeneficiariosfec_afiliacion_uni.value:= strtodate('01'+copy(datetostr(frmafiliaciones.fecha_actual()),3,8));
...
procedure Tfrmafiliacion_usuarios.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
if (TabSheet1.tabVisible = true) and ( TabSheet5.TabVisible = true ) then
begin
      Query1.Close;
      Query1.SQL.Clear;
      Query1.SQL.Add('Select * '+
...
procedure Tfrmafiliacion_usuarios.btnempresasClick(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBGrid2EditButtonClick(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.qryafiliados_empresascodocupacionChange(
  Sender: TField);
begin
        dmafiliaciones.tblocupaciones.refresh;
end;

procedure Tfrmafiliacion_usuarios.qryafiliados_empresastipodocumentoempChange(
  Sender: TField);
begin
        dmafiliaciones.tbltipos_documentos.Refresh;
end;

procedure Tfrmafiliacion_usuarios.tblafiliados1AfterPost(DataSet: TDataSet);
begin
        ...
        //        Query1.ExecSQL;
                ...
                begin
                    ...
                    Query1.Params[2].AsString := tblafiliadostip_documento.text;
                    Query1.Params[3].AsString := tblafiliadosnum_documento.text;
                    Query1.Params[4].AsDate := strtodate(DBEdit30.text);
...
procedure Tfrmafiliacion_usuarios.PageControl1Change(Sender: TObject);
begin
{if PageControl1.Activepage = TabSheet5 then
begin
if DBEdit34.Text <> '' then
  begin
   Label26.caption:=frmafiliacion_usuarios.Caption;
   end
   else
...
procedure Tfrmafiliacion_usuarios.tblafiliados1BeforePost(
  DataSet: TDataSet);
begin
        tblafiliadosfec_ultima_nov.value:=frmafiliaciones.fecha_actual();
        tblafiliadoscod_discapacidad.Value :='N';
//        tblafiliadoscod_sexo.Value := dblookupcombobox6.DataField;
//        if TabSheet1.TabVisible = true then
  //      begin
    //      IF
//          tblafiliadoscod_modalidad_tra.Value :='D';
      //  end;


end;

procedure Tfrmafiliacion_usuarios.qryafiliados_empresasUpdateError(
...
procedure Tfrmafiliacion_usuarios.qryafiliados_empresasAfterInsert(
  DataSet: TDataSet);
begin
...
procedure Tfrmafiliacion_usuarios.Button1Click(Sender: TObject);
begin
        frmasesores:=Tfrmasesores.create(Application);
              try
              frmasesores.Caption:='Selecion Asesores  para dependientes';
                      frmasesores.show;
...
procedure Tfrmafiliacion_usuarios.Button2Click(Sender: TObject);
begin
        frmeps:=Tfrmeps.create(Application);
              try
                      frmeps.Caption:='Seleccion de la eps para dependientes';
                      frmeps.show;
...
procedure Tfrmafiliacion_usuarios.FormCreate(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.qryafiliadosAfterPost(DataSet: TDataSet);
begin
        Query1.Close;
        Query1.SQL.Clear;
        Query1.SQL.Add('Select dbo.relacion_afiliados.tip_documento_cot,'+
...
procedure Tfrmafiliacion_usuarios.qryafiliadosBeforePost(DataSet: TDataSet);
begin
        tblafiliadosfec_ultima_nov.value:=frmafiliaciones.fecha_actual();
...
procedure Tfrmafiliacion_usuarios.btngrabarClick(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit22Exit(Sender: TObject);
var
...
//          tblafiliados.Findkey([tipo_doc,num_doc]);
          ...
          begin
              ...
              //   btnmodo_insertarClick(frmafiliacion_usuarios);
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox1CloseUp(
  Sender: TObject);
begin
        dmafiliaciones.tbltipos_documentos.Refresh;
end;

procedure Tfrmafiliacion_usuarios.DateTimePicker1Change(Sender: TObject);
begin
        tblafiliados.edit;
        tblafiliadosfec_radicacion.Value:=DateTimePicker1.date;
end;

procedure Tfrmafiliacion_usuarios.DateTimePicker2Change(Sender: TObject);
begin
        tblafiliados.edit;
        tblafiliadosfec_nacimiento.Value:=DateTimePicker2.date;
end;

procedure Tfrmafiliacion_usuarios.dbldepartamentosClick(Sender: TObject);
begin
        dmafiliaciones.Qryciudades.Active:=false;
        dmafiliaciones.Qryciudades.Params[0].AsString := tblafiliadoscod_departamento_res.text;
        dmafiliaciones.Qryciudades.ExecSQL;
...
procedure Tfrmafiliacion_usuarios.dblciudadesClick(Sender: TObject);
begin
        dmafiliaciones.qrybarrios.Active:=false;
        dmafiliaciones.qrybarrios.Params[0].AsString := tblafiliadoscod_departamento_res.text;
        dmafiliaciones.qrybarrios.Params[1].AsString := tblafiliadoscod_ciudad_res.text;
...
procedure Tfrmafiliacion_usuarios.DateTimePicker5Change(Sender: TObject);
begin
        qrybeneficiarios.edit;
        qrybeneficiariosfec_nacimiento.Value:=DateTimePicker5.date;
end;

procedure Tfrmafiliacion_usuarios.qryafiliados_empresasBeforeInsert(
  DataSet: TDataSet);
begin
        afiliados_empresas_insert:=true;
end;

procedure Tfrmafiliacion_usuarios.qryafiliados_empresasBeforeCancel(
  DataSet: TDataSet);
begin
...
procedure Tfrmafiliacion_usuarios.Button3Click(Sender: TObject);
begin
        frmips:=Tfrmips.create(Application);
              try
                      if tblafiliadosCOD_TIPO_AFIL.Text = 'C' then
                      frmips.caption:='[IPS]- COTIZANTE'
                      else
                      frmips.caption:='[IPS]- BENEF.';
...
procedure Tfrmafiliacion_usuarios.Button4Click(Sender: TObject);
begin
        frmips:=Tfrmips.create(Application);
              try
                      frmips.caption:='[IPS]- BENEFICIARIO';
                      frmips.eddepto.text:=tblafiliadoscod_departamento_res.Text;
                      frmips.edmun.text:=tblafiliadoscod_ciudad_res.Text;
                      frmips.show;
              except
...
procedure Tfrmafiliacion_usuarios.btnmodo_insertarClick(Sender: TObject);
begin
      Query1.Close;
      Query1.SQL.Clear;
      Query1.SQL.Add('Select * '+
...
procedure Tfrmafiliacion_usuarios.qryrelacion_afiliadosPostError(
...
procedure Tfrmafiliacion_usuarios.Button20Click(Sender: TObject);
begin
      ...
      tblafiliadosfec_radicacion.Value:=frmafiliacion_usuarios.DateTimePicker1.Date;
      tblafiliadosfec_nacimiento.Value:=frmafiliacion_usuarios.DateTimePicker2.Date;
      tblafiliadosfec_afiliacion_sis.Value:=frmafiliacion_usuarios.DateTimePicker3.Date;
      tblafiliadosfec_afiliacion_uni.Value:=strtodate(frmafiliacion_usuarios.DateTimePicker4.text);
     // PageControl1.ActivePage:=frmafiliacion_usuarios.TabSheet2;
end;

procedure Tfrmafiliacion_usuarios.DateTimePicker6Change(Sender: TObject);
begin
qrybeneficiarios.edit;
qrybeneficiariosfec_afiliacion_sis.Value:=DateTimePicker6.date;
end;

procedure Tfrmafiliacion_usuarios.DateTimePicker7Change(Sender: TObject);
begin
     qrybeneficiarios.edit;
     qrybeneficiariosfec_afiliacion_uni.Value:=DateTimePicker7.date;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox9Click(Sender: TObject);
begin
        dmafiliaciones.Qryciudades.Active:=false;
        dmafiliaciones.Qryciudades.Params[0].AsString := qrybeneficiarioscod_departamento_res.text;
        dmafiliaciones.Qryciudades.ExecSQL;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox10Click(Sender: TObject);
begin
        dmafiliaciones.qrybarrios.Active:=false;
        dmafiliaciones.qrybarrios.Params[0].AsString := qrybeneficiarioscod_departamento_res.text;
        dmafiliaciones.qrybarrios.Params[1].AsString := qrybeneficiarioscod_ciudad_res.text;
...
procedure Tfrmafiliacion_usuarios.DBEdit23KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.numerosKeypress(dbedit23,Key);
end;

procedure Tfrmafiliacion_usuarios.AppMessage(var Msg: TMsg;
  var Handled: Boolean);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit31KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.letrasKeypress(dbedit31,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit32KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.letrasKeypress(dbedit32,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit28KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.letrasKeypress(dbedit28,Key);   
end;

procedure Tfrmafiliacion_usuarios.DBEdit29KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.letrasKeypress(dbedit29,Key);   
end;

procedure Tfrmafiliacion_usuarios.DBEdit6KeyPress(Sender: TObject;
  var Key: Char);
begin
//     frmafiliaciones.numerosKeypress(dbedit6,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit4KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit4,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit3KeyPress(Sender: TObject;
  var Key: Char);
begin
      frmafiliaciones.numerosKeypress(dbedit3,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit20KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.numerosKeypress(dbedit20,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit12KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit12,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit13KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit13,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit14KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit14,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit26KeyPress(Sender: TObject;
  var Key: Char);
begin
         frmafiliaciones.numerosKeypress(dbedit6,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit33KeyPress(Sender: TObject;
  var Key: Char);
begin
      //  frmafiliaciones.numerosKeypress(dbedit33,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit34KeyPress(Sender: TObject;
  var Key: Char);
begin
       //frmafiliaciones.numerosKeypress(dbedit34Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit5KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit5,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit7KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.letrasKeypress(dbedit7,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit8KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.letrasKeypress(dbedit8,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit9KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.letrasKeypress(dbedit9,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit10KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.letrasKeypress(dbedit10,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit17KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit17,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit18KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit18,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit27KeyPress(Sender: TObject;
  var Key: Char);
begin
     frmafiliaciones.numerosKeypress(dbedit27,Key);
end;

procedure Tfrmafiliacion_usuarios.Button6Click(Sender: TObject);
begin
     frmeps:=Tfrmeps.create(Application);
              try
                      frmeps.Caption:='Seleccion de la eps para el Beneficiario dependiente';
                      frmeps.show;
...
procedure Tfrmafiliacion_usuarios.DBEdit24KeyPress(Sender: TObject;
  var Key: Char);
begin
        frmafiliaciones.numerosKeypress(dbedit24,Key);
end;

procedure Tfrmafiliacion_usuarios.tblafiliados1AfterInsert(
  DataSet: TDataSet);
begin
     afiliados_insert:=true;
end;

procedure Tfrmafiliacion_usuarios.DateTimePicker2Exit(Sender: TObject);
var yy : real;
begin
     ...
     begin
      ...
      if (DBLookupComboBox2.KeyValue = 'NIT') then
        ...
        DBLookupComboBox2.SetFocus;
             ...
             if  YearSpan(gb_fec_hoy,DateTimePicker2.Date) <= 18 then
             begin
...
procedure Tfrmafiliacion_usuarios.DateTimePicker1Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DateTimePicker3Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DateTimePicker5Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DateTimePicker6Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBGrid1ColExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit6Exit(Sender: TObject);
begin
  ...
  exit;
      ...
      Query1.SQL.Add('Select * '+
      'From afiliados '+
      'Where afiliados.num_formato_afil = '+#39+trim(DBEdit6.Text)+#39);
      Query1.Open;
...
procedure Tfrmafiliacion_usuarios.PageControl3Change(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit31Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox2Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblsexoExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit28Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblestratoExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblestadocivilExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblmodalidadtrabajoExit(Sender: TObject);
begin
if trim(dblmodalidadtrabajo.Text) = '' then
 ...
 tblafiliadoscod_modalidad_tra.Value := dblmodalidadtrabajo.KeyValue;

 if frmafiliacion_usuarios.DBNavigator3.Visible = false then
    Button9.Click;
 end;

{if trim(dblmodalidadtrabajo.Text) = '' then
  begin
  ShowMessage('Selecione  el tipo de trabajo del afiliado');
  dblmodalidadtrabajo.SetFocus;
  end
  else
    if dblmodalidadtrabajo.KeyValue = 'D' then
    begin
      if DateTimePicker2.DateTime > gb_fec_hoy - 6570 then
      begin
           if MessageDlg('Afiliado Dependiente  menor de Edad Desea Ingresarlo ?', mtConfirmation, [mbYes, mbNo], 0) = mrno then
           begin
           DateTimePicker2.SetFocus;
          end;
      end;
    ...
    begin
      if DateTimePicker2.DateTime > gb_fec_hoy - 6570 then
      begin
           if MessageDlg('Empleada Domestica menor de Edad Desea Ingresarlo ?', mtConfirmation, [mbYes, mbNo], 0) = mrno then
           begin
           DateTimePicker2.SetFocus;
          end;
      end;
    ...
    begin
      if DateTimePicker2.DateTime > gb_fec_hoy - 6570 then
      begin
           if MessageDlg('Independiente Menor de Edad Desea Ingresarlo ?', mtConfirmation, [mbYes, mbNo], 0) = mrno then
           begin
           DateTimePicker2.SetFocus;
          end;
      end;
    ...
    begin
      if DateTimePicker2.DateTime > gb_fec_hoy - 6570 then
      begin
           if MessageDlg('Madre Comunitaria  menor de Edad Desea Ingresarlo ?', mtConfirmation, [mbYes, mbNo], 0) = mrno then
           begin
           DateTimePicker2.SetFocus;
          end;
      end;
    ...
    begin
      if DateTimePicker2.DateTime > gb_fec_hoy - 6570 then
      begin
           if MessageDlg('Afiliado Pensionado Menor menor de Edad Desea Ingresarlo ?', mtConfirmation, [mbYes, mbNo], 0) = mrno then
           begin
           DateTimePicker2.SetFocus;
          end;
      end;
...
procedure Tfrmafiliacion_usuarios.dbldepartamentosExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblciudadesExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblbarriosExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox3Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit11Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit15Exit(Sender: TObject);
var log,i : integer;
arroba : string;
encontro : Boolean;
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit12Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblescolaridadExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dblprofesionExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox1Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox5Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit3Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit20Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox4Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox6Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox8Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.dlbtipoafiliadoExit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit7Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit9Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.Button7Click(Sender: TObject);
begin
     frmempresas:=Tfrmempresas.create(Application);
              try
              frmempresas.caption := 'Empresas Activas dependientes';
                      frmempresas.show;
...
procedure Tfrmafiliacion_usuarios.DBEdit5Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox17Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox18Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.qryrelacion_afiliadosBeforeCancel(
  DataSet: TDataSet);
begin
        qrybeneficiarios.cancel;
        DBLookupComboBox4.setfocus;
...
procedure Tfrmafiliacion_usuarios.Button8Click(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox10Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox9Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox11Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox7Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit16Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit17Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DateTimePicker8Change(Sender: TObject);
begin
 qryafiliados_empresas.edit;
 qryafiliados_empresasFEC_ING_EMPRESA.Value:=DateTimePicker8.date;
end;

procedure Tfrmafiliacion_usuarios.DBEdit36Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBGrid2Enter(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.grabar_afiliados_cotizantes;

begin

             try
                   tblafiliados.edit;
                   tblafiliadosfec_radicacion.Value:=frmafiliaciones.fecha_hoy();
//                   tblafiliadosfec_radicacion.Value:=DateTimePicker1.date;
                   ...
                   qryrelacion_afiliados.Active:=false;
                    ...
                    qryrelacion_afiliados.RecordCount;
                    qryrelacion_afiliados.insert;
                    //DBLookupComboBox4.SetFocus();
                    qrybeneficiarios.Active:=false;
                    Qrybeneficiarios.Params[0].AsString := qryrelacion_afiliadostip_documento_ben.Text;
...
procedure Tfrmafiliacion_usuarios.Button9Click(Sender: TObject);
begin
...
begin
      ...
      qrynovedades_afiliados.Cancel;
      qrynovedades_afiliados.free;
      if frmafiliacion_usuarios = nil then
      begin
      frmafiliacion_usuarios.Free;
      end
      else
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox18Enter(Sender: TObject);
begin
  if  DBLookupComboBox17.KeyValue = 'C' then
   begin
   qryrelacion_afiliados.Edit;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox22Click(Sender: TObject);
begin
        dmafiliaciones.Qryciudades2.Active:=false;
        dmafiliaciones.qryciudades2.Params[0].AsString := qryafiliados_empresascod_departamento_lab.Value;
        dmafiliaciones.Qryciudades2.ExecSQL;
...
procedure Tfrmafiliacion_usuarios.DBEdit59Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit60Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit36KeyPress(Sender: TObject;
  var Key: Char);
begin
 frmafiliaciones.numerosKeypress(dbedit36,Key);
end;

procedure Tfrmafiliacion_usuarios.DBEdit35KeyPress(Sender: TObject;
  var Key: Char);
begin
 frmafiliaciones.numerosKeypress(dbedit35,Key);
end;

function Tfrmafiliacion_usuarios.validar_salario: Boolean;
  begin
...
procedure Tfrmafiliacion_usuarios.DBEdit58Exit(Sender: TObject);
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox22Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox23Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox24Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox21Exit(Sender: TObject);
begin
if  DBLookupComboBox21.Text = '' then
   begin
   ShowMessage('Selecione El tipo de Salario  que tendra  el afiliado ');
   DBLookupComboBox21.SetFocus;
   end
//   else
 //  if  DBLookupComboBox21.Text <> '' then
  // begin
//        dmafiliaciones.qrysalariominimo.Active:=false;
 //       dmafiliaciones.qrysalariominimo.Params[0].AsString := '01/01/'+ (FORMATDATETIME('YYYY',strtodate(DateTimePicker4.text)));
...
procedure Tfrmafiliacion_usuarios.DateTimePicker8Exit(Sender: TObject);
begin
   if strtodate(DBEdit39.Text) > DateTimePicker8.DateTime then
        begin
        ShowMessage('La Fecha  de Ingreso a la Empresa  debe ser mayo a la fecha  de Nacimiento');
        DateTimePicker8.SetFocus;
        end
        else
        if gb_fec_hoy < DateTimePicker8.DateTime then
        begin
...
procedure Tfrmafiliacion_usuarios.FormActivate(Sender: TObject);
begin
Button9.Enabled := true;
end;

procedure Tfrmafiliacion_usuarios.btnnuevoClick(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox15Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit24Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit27Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.btncempresaClick(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.btnnempresaClick(Sender: TObject);
begin
DBNavigator2.BtnClick(nbInsert);
btnnempresa.Visible := false;
btngempresa.Visible := true;
...
procedure Tfrmafiliacion_usuarios.btngempresaClick(Sender: TObject);
begin
...
begin
        ...
        btnnempresa.Visible := false;
        btngempresa.Visible := false;
        if tabsheet5.Enabled = true then
        begin
            PageControl3.ActivePage := TabSheet5;
        ...
        ELSE
        begin
        if TabSheet3.TabVisible = false then
        begin
      Query1.close;
      qryrelacion_afiliados.close;
      qryrelacion_afiliados.free;
...
procedure Tfrmafiliacion_usuarios.validar_grupos;
begin
    if dlbtipoafiliado.KeyValue = 'B' then
    begin
             if (DBLookupComboBox17.KeyValue = 'P') then
...
procedure Tfrmafiliacion_usuarios.RadioGroup1Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.Primeravezeps;
begin
...
procedure Tfrmafiliacion_usuarios.RadioGroup1Click(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.TRANSLADOEPS;
begin
 if  (FORMATDATETIME('MM',(DateTimePicker1.DATE)) = '12') then
      begin
...
procedure Tfrmafiliacion_usuarios.SEMANAS_COTIZADAS;
VAR fecha :tdate;
    i : real;
begin
...
procedure Tfrmafiliacion_usuarios.RadioGroup2Exit(Sender: TObject);
begin
// si no escoje  el tipo de afiliacion
...
procedure Tfrmafiliacion_usuarios.RadioGroup2Click(Sender: TObject);
begin
 // si no viene  de  translado
       if RadioGroup2.ItemIndex = 0 then
       begin
   ...
   if tblafiliadosFEC_AFILIACION_UNI.value > DateTimePicker7.Date then
        DateTimePicker7.Date:=tblafiliadosFEC_AFILIACION_UNI.value;

end;

function Tfrmafiliacion_usuarios.validar_datos_cotizante: Boolean;
begin
validar_datos_cotizante := false;
          if dblsexo.KeyValue = '' then
          begin
        ...
        if dblestadocivil.KeyValue = '' then
          begin
       ...
       { if dblmodalidadtrabajo.KeyValue = '' then
          begin
          ...
          if dbldepartamentos.KeyValue = '' then
          begin
          ...
          begin
           ...
           if dblescolaridad.KeyValue = '' then
          begin
...
function Tfrmafiliacion_usuarios.validar_datos_empresa: Boolean;
begin
      validar_datos_empresa := false;
          if DBLookupComboBox25.KeyValue = '' then
          begin
          ...
          if DBLookupComboBox19.KeyValue = '' then
          begin
          ...
          if DBLookupComboBox21.KeyValue = '' then
          begin
              ShowMessage('Antes  de Continuar debe selecionar  el Tipo de Salario del Afiliado en la Empresa');
              validar_datos_empresa := true;
              DBLookupComboBox21.SetFocus;
              exit;
          end;
          ...
          if DBLookupComboBox22.KeyValue = '' then
          begin
          ...
          if DBLookupComboBox23.KeyValue = '' then
          begin
          ...
          if DBLookupComboBox24.KeyValue = '' then
          begin
...
procedure Tfrmafiliacion_usuarios.pasar_cotizante_Inactivo_Activo;
begin
  dmafiliaciones.softeps.StartTransaction;
  try
  tblafiliados.edit;
...
procedure Tfrmafiliacion_usuarios.ingresar_multiempresa;
begin
  qryafiliados_empresas.Close;
  qryafiliados_empresas.open;
  dmafiliaciones.Qryciudades2.Active:=false;
...
procedure Tfrmafiliacion_usuarios.pasar_beneficiario_cotizante_inactivo;
begin
...
procedure Tfrmafiliacion_usuarios.DBEdit35Exit(Sender: TObject);
begin
if  DBEdit35.Text = '' then
   begin
       ...
       qryafiliados_empresascod_sucursal.Value:=qryempresasCOD_SUCURSAL.Value;
        ...
        dbctividadeconomica.KeyValue := qryempresasCOD_ACTIVIDAD_ECO.Value;
            ...
            qryarp.OPEN;
             ...
             begin
             Button7.Enabled := false;
             DateTimePicker8.Date := strtodate(DateTimePicker4.text);
             qryafiliados_empresas.edit;
...
procedure Tfrmafiliacion_usuarios.DateTimePicker7Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.ingreso_adicional;
begin
      qryvalrealcionafiliados.CLOSE;
      qryvalrealcionafiliados.PREPARE;
      qryvalrealcionafiliados.SQL.CLEAR;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox19Exit(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox25Exit(Sender: TObject);
begin
if  DBLookupComboBox2.Text = '' then
   begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox17Enter(Sender: TObject);
begin
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox8Enter(Sender: TObject);
begin
qrybeneficiarios.Edit;
...
procedure Tfrmafiliacion_usuarios.dlbtipoafiliadoEnter(Sender: TObject);
begin
qrybeneficiarios.Edit;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox6Enter(Sender: TObject);
begin
// if DBLookupComboBox6.KeyValue = 'M' then
//  begin
//  qrybeneficiarios.Edit;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox14CloseUp(
  Sender: TObject);
begin
dmafiliaciones.tbltipos_documentos.Refresh;
end;

procedure Tfrmafiliacion_usuarios.DBLookupComboBox16CloseUp(
  Sender: TObject);
begin
dmafiliaciones.tbltipos_documentos.Refresh;
end;

procedure Tfrmafiliacion_usuarios.DBLookupComboBox29Exit(Sender: TObject);
begin
if  DBLookupComboBox3.Text = '' then
   begin
   end;
...
procedure Tfrmafiliacion_usuarios.Button5Click(Sender: TObject);
begin
frmeps:=Tfrmeps.create(Application);
              try
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox32Click(Sender: TObject);
begin
        dmafiliaciones.Qryciudades.Active:=false;
        dmafiliaciones.Qryciudades.Params[0].AsString := tblafiliadoscod_departamento_res.text;
        dmafiliaciones.Qryciudades.ExecSQL;
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox33Click(Sender: TObject);
begin
        dmafiliaciones.qrybarrios.Active:=false;
        dmafiliaciones.qrybarrios.Params[0].AsString := tblafiliadoscod_departamento_res.text;
        dmafiliaciones.qrybarrios.Params[1].AsString := tblafiliadoscod_ciudad_res.text;
...
procedure Tfrmafiliacion_usuarios.DBEdit4Exit(Sender: TObject);
begin
if  DBEdit4.Text = '' then
   begin
   ShowMessage('Digite el Numero de  documento del Asesor Comercial que realizo la afiliacion');
   DBEdit4.SetFocus;
   end
   else
   qry.close;
...
procedure Tfrmafiliacion_usuarios.DBEdit19Exit(Sender: TObject);

begin
      if DBEdit19.text = '' then
      begin
          ShowMessage('Digite el numero de  Documento de la IPS');
          DBEdit19.SetFocus;
      end
      else
   qry2.close;
   qry2.sql.clear;
   qry2.sql.add('select * from prestadores where tip_identificacion = :"tipo" and num_identificacion = :"numero"');
   qry2.PARAMS[0].ASSTRING := DBLookupComboBox13.KeyValue;
   qry2.PARAMS[1].ASSTRING := DBEdit19.Text;

   qry2.open;
   if qry2.RecordCount = 1 then
   begin
   Edips.Text := qry2mon_prestador.Value;
   ...
   ShowMessage('Verifique  el docuemento del la IPS o  escojalo de  la lista');
   Button3.Enabled := false;
              frmips:=Tfrmips.create(Application);
              try
                      frmips.caption:='[IPS]- COTIZANTE';
...
procedure Tfrmafiliacion_usuarios.DBLookupComboBox13Exit(Sender: TObject);
begin
