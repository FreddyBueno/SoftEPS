...
USES DTMODULO, UACERCA, UAFILIADOS, UEMPRESAS, UPRINCIPAL,
  UGLOBAL,UCONPAGOS, UCONPAGOSE, UAFILIAEMPRESA;
...
IF ((EDTNUMERODUCAFILIADO.TEXT <> '') AND (CBTIPODOCAFILIADO.TEXT <> ''))AND ( EDTCOTIZACIONOBLIGATORIO.TEXT > '0') THEN
 ...
 IF  EDTTOTALPAGADOAFILIADO.TEXT = '' THEN
   ...
   SGDETALLE.CELLS[23,SGDETALLE.ROWCOUNT -2] := estado_pago ;
...
EMP := FALSE;
    ...
    // VERIFICAR  EL DOCUMENTO EN LA LISTA
      ...
      // EDTSALARIOBASE.ENABLED := FALSE;
       ...
       EDTAUTINCAPACIDAD.TEXT := SGDETALLE.CELLS[17,I ];
       EDTVALINCAPACIDAD.TEXT := SGDETALLE.CELLS[18,I ];
       EDTNOAUTLICENCIA.TEXT := SGDETALLE.CELLS[19,I ];
       EDTVALLICENCIA.TEXT := SGDETALLE.CELLS[20,I ];
           ...
           IF EDTNUMERODUCAFILIADO.TEXT > '0' THEN
                ...
                BEGIN
                      ...
                      BEGIN
                        ...
                        ELSE
                        BEGIN
                          IF MESSAGEDLG (LowerCase('!USUARIO NO PERTENCE A  ESTA  EMPRESA ? DESEA  CONTINUA'), MTCONFIRMATION, [MBYES, MBNO], 0) = mrYes THEN
                          BEGIN
                          EDTNOMBREAFILIADO.TEXT := DATAMODULE1.QRYAFILIADOSPRI_APELLIDO.VALUE + ' ' + DATAMODULE1.QRYAFILIADOSSEG_APELLIDO.VALUE + ' ' + DATAMODULE1.QRYAFILIADOSPRI_NOMBRE.VALUE + ' ' + DATAMODULE1.QRYAFILIADOSSEG_NOMBRE.VALUE;
                          EDTSALARIOBASE.TEXT := FLOATTOSTR(DATAMODULE1.QRYAFILIADOSVAL_SUELDO_AFIL.VALUE);
                              IF MODIFICADO = FALSE THEN
...
PROCEDURE TFRMPLANILLAS.EDTPLANILLAKEYPRESS(SENDER: TOBJECT;
  VAR KEY: CHAR);
BEGIN
IF NOT (KEY IN ['0'..'9',#8])THEN
  BEGIN
 ...
 CANTIDAD1 := 0;
        ...
        IF DATAMODULE1.QRYAUTOLIQUIDACION.FIELDBYNAME('TIP_DECLARACION').VALUE  = 'D' THEN
         ...
         ELSE IF DATAMODULE1.QRYAUTOLIQUIDACION.FIELDBYNAME('TIP_DECLARACION').VALUE  = 'A' THEN
                ...
                CBTIPODECLARACION.ITEMINDEX := 1;
                END
                ELSE IF DATAMODULE1.QRYAUTOLIQUIDACION.FIELDBYNAME('TIP_DECLARACION').VALUE  = 'C' THEN
                        BEGIN
                        CBTIPODECLARACION.ITEMINDEX := 2;
                        END;
...
BEGIN
  ...
  IF (GLSALARIOMINIMO * 20) < (strtofloat(edtigresobasecotizacion.TEXT)/ STRTOINT(EDDIAS.TEXT)*30) THEN
   begin
   EDTCOTIZACIONOBLIGATORIO.setfocus;
 ...
 IBC := FALSE;
      IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'D' THEN
      BEGIN
           IF  ROUNDTO(((GLSALARIOMINIMO)/30)* STRTOINT(EDDIAS.TEXT),0) >  (strtofloat(edtigresobasecotizacion.TEXT)/ STRTOINT(EDDIAS.TEXT)*30) THEN
           BEGIN
               IF MESSAGEDLG(LowerCase('!EL INGRESO BASE DE  COTIZACION  PARA DEPENDIENTES ES  DE UN SALARIO MINIMO ¡ DESEA  INGRESARLO ?'), MTCONFIRMATION, [MBYES, MBNO], 0) = MRYES THEN
                BEGIN
                IBC := TRUE;
                END ELSE EDTCOTIZACIONOBLIGATORIO.SETFOCUS;
           END;
      END
      ELSE IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'E' THEN
           BEGIN
                IF  ROUNDTO(((GLSALARIOMINIMO/2)/30)* STRTOINT(EDDIAS.TEXT),0) >  (strtofloat(edtigresobasecotizacion.TEXT)/ STRTOINT(EDDIAS.TEXT)*30) THEN
                BEGIN
                     IF MESSAGEDLG(LowerCase('!EL INGRESO BASE DE  COTIZACION  PARA EMPLEADAS  DOMESTICAS ES  DE UN SALARIO MINIMO ¡ DESEA  INGRESARLO ?'), MTCONFIRMATION, [MBYES, MBNO], 0) = MRYES THEN
                     BEGIN
                     IBC := TRUE;
                     END ELSE EDTCOTIZACIONOBLIGATORIO.SETFOCUS;
                     END;
      ...
      ELSE IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'P' THEN
           BEGIN
                IF  ROUNDTO(((GLSALARIOMINIMO)/30)* STRTOINT(EDDIAS.TEXT),0) >  (strtofloat(edtigresobasecotizacion.TEXT)/ STRTOINT(EDDIAS.TEXT)*30) THEN
                BEGIN
                     IF MESSAGEDLG(LowerCase('!EL INGRESO BASE DE  COTIZACION  PARA EMPLEADAS  PENSIONADO ES  DE UN SALARIO MINIMO ¡ DESEA  INGRESARLO ?'), MTCONFIRMATION, [MBYES, MBNO], 0) = MRYES THEN
                     BEGIN
                     IBC := TRUE;
                     END ELSE EDTCOTIZACIONOBLIGATORIO.SETFOCUS;
                     END;
     ...
     ELSE IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'I' THEN
           BEGIN
                IF  ROUNDTO((((GLSALARIOMINIMO*2))/30)* STRTOINT(EDDIAS.TEXT),0) >  (strtofloat(edtigresobasecotizacion.TEXT)/ STRTOINT(EDDIAS.TEXT)*30) THEN
                BEGIN
                     IF MESSAGEDLG(LowerCase('!EL INGRESO BASE DE  COTIZACION  PARA INDEPENDIENTES ES  DE DOS  SALARIO MINIMO ¡ DESEA  INGRESARLO ?'), MTCONFIRMATION, [MBYES, MBNO], 0) = MRYES THEN
                     BEGIN
                     IBC := TRUE;
                     END ELSE EDTCOTIZACIONOBLIGATORIO.SETFOCUS;
                     END;
...
IF MESSAGEDLG(LowerCase('DESEA  BORRAR LA PLANILLA  DE  AUTOLIQUIDACION ?'), MTCONFIRMATION, [MBYES, MBNO], 1) = MRYES THEN
  ...
  TRY
         ...
         DATAMODULE1.qrydetallepago.PARAMS[0].ASSTRING := TRIM(EDTPLANILLA.TEXT);
         DATAMODULE1.qrydetallepago.REQUESTLIVE := TRUE;
         ...
         DATAMODULE1.QRYUPC.PARAMS[0].ASSTRING := TRIM(EDTPLANILLA.TEXT);
         DATAMODULE1.QRYUPC.REQUESTLIVE := TRUE;
         DATAMODULE1.QRYUPC.EXECSQL;
          ...
          DATAMODULE1.QRYAUTOLIQUIDACION.EXECSQL;
           ...
           DataModule1.qryestrato.Params[2].Asdate := dtfechapago.Date;
           DataModule1.qryestrato.Params[3].AsFloat := strtofloat(edttotalconsignado.text);
           DataModule1.qryestrato.Params[4].AsString := 'S';
           DataModule1.qryestrato.RequestLive := true;
           DataModule1.qryestrato.Open;
                 if DataModule1.qryestrato.RecordCount = 1 then
                 begin
...
procedure TFRMPLANILLAS.RETIRAR_USUARIOS;
var i,x : Integer;
begin
DataModule1.qrydetalle.Close;
DataModule1.qrydetalle.Prepare;
...
//                                        DataModule1.qrynovafiliados.Params[18].AsString := 'NOV_AFILIADOS.NEXTVAL';
                            ...
                            DataModule1.dtbdatamec.Rollback;
                            SHOWMESSAGE(LowerCase('La  planilla  fue grabada  correctamente pero no se puede  Retirar el Afiliado'));
                            LIMPIAR_PLANILLA;
                            raise;
                           end;
                           end;
...
IF EDTCOTIZACIONOBLIGATORIO.TEXT > '0' THEN
    BEGIN
        ...
        BEGIN
        EDTIGRESOBASECOTIZACION.TEXT :=   FLOATTOSTR(ROUNDTO((STRTOINT(EDTCOTIZACIONOBLIGATORIO.TEXT)/ 0.12),3));
        ...
        // verifcar  si no es  NRS Nuevamente
                ...
                IF DATAMODULE1.QRYAFILIADOS.RECORDCOUNT > 0 THEN
                   BEGIN
                        ...
                        DATAMODULE1.QRYAFILIADOS.PARAMS[4].AsDate := STRTODATE('01/' + EDTPERIODO.TEXT);
                        DATAMODULE1.QRYAFILIADOS.PARAMS[5].AsDate := STRTODATE('28/' + EDTPERIODO.TEXT);
                        DATAMODULE1.QRYAFILIADOS.PARAMS[6].ASSTRING := TRIM(cbtipodocempresa.TEXT);
                        DATAMODULE1.QRYAFILIADOS.PARAMS[7].ASSTRING := TRIM(ednumero.TEXT);
                        DATAMODULE1.QRYAFILIADOS.OPEN;
                        IF DATAMODULE1.QRYAFILIADOS.RECORDCOUNT = 0 THEN
                           BEGIN
                          estado_pago := 'EMP';
        ...
        ELSE    IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'P' THEN
                ...
                CBTIPOCOTIZANTE.ITEMINDEX := 2;
                 END  ELSE  IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'E' THEN
                            ...
                            CBTIPOCOTIZANTE.ITEMINDEX := 3;
                            END
                            ELSE  IF DATAMODULE1.QRYAFILIADOSCOD_MODALIDAD_TRA.VALUE = 'M' THEN
                                  BEGIN
                                  CBTIPOCOTIZANTE.ITEMINDEX := 4;
                                  END;
